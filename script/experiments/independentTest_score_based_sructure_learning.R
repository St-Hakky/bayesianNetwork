...
getBlacklistWithFisher <- function(data, p.value=0.05){
  from.vec = c()
  to.vec = c()
    ...
    for(j in 1:length(names(data))){
      if(i != j){
        ...
        if(res.fisher$p.value >= p.value){
          from.vec = append(from.vec, names(data)[i])
          to.vec = append(to.vec, names(data)[j])
        }
      }
    }
  }
  df = data.frame(from=from.vec, to=to.vec)
  return(df)
}

learnBN <- function(data, data.size.vec, try.size, correct.graph.string, flag.skeleton=FALSE){
  ...
  for(data.size in data.size.vec){
    ...
    for(i in 1:try.size){
      ...
      res.blacklist = getBlacklistWithFisher(sample.data)
      res.learn = hc(sample.data, score="bic",blacklist=res.blacklist)
...
main <- function(){
  ...
  result = learnBN(insurance, data.size.vec, try.size, correct.graph.string,flag.skeleton = FALSE)
