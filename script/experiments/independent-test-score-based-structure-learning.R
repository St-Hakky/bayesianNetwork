...
plotFPResult <- function(data_size_vec, data_name, fp_avg_vec, fp_sd_vec = NULL,
  ...
  if (flag_error_bar) {
    g <- g + geom_errorbar(aes(x = id, y = fp_avg, ymin = fp_avg - fp_sd, ymax = fp_avg +
      fp_sd), width = 1)
    g <- g + geom_point()
  }
  g <- g + geom_line()
  g <- g + coord_cartesian(xlim = c(0, max(data_size_vec))) + ggtitle(paste("fp-avg-Data :",
    data_name))
  plot(g)
}

plotPrecisionResult <- function(data_size_vec, data_name, precision, precision_sd_vec = NULL,
  flag_error_bar = FALSE) {
  ...
  if (flag_error_bar) {
    g <- g + geom_errorbar(aes(x = id, y = precision, ymin = precision - precision_sd,
      ymax = precision + precision_sd), width = 1)
    g <- g + geom_point()
  }
  g <- g + geom_line()
...
plotRecallResult <- function(data_size_vec, data_name, recall, recall_sd_vec = NULL,
  flag_error_bar = FALSE) {
  ...
  if (flag_error_bar) {
    g <- g + geom_errorbar(aes(x = id, y = recall, ymin = recall - recall_sd,
      ymax = recall + recall_sd), width = 1)
    g <- g + geom_point()
  }
  g <- g + geom_line()
...
getStructureWithFisher <- function(data, p_value = 0.05) {
...
learnBN <- function(data, data_size_vec, try_size, correct_graph_string) {
  ...
  return(list(tp_avg_vec, fp_avg_vec, fn_avg_vec, tp_sd_vec, fp_sd_vec, fn_sd_vec,
    precision_avg_vec, precision_sd_vec))
}
