...
getBlacklistWithFisher <- function(data, p_value = 0.05) {
  from_vec <- c()
  to_vec <- c()
    ...
    for (j in 1:length(names(data))) {
      if (i != j) {
        ...
        if (res_fisher$p.value >= p_value) {
          from_vec <- append(from_vec, names(data)[i])
          to_vec <- append(to_vec, names(data)[j])
        }
      }
    }
  }
  df <- data.frame(from = from_vec, to = to_vec)
  return(df)
}

learnBN <- function(data,
                    data_size_vec,
                    try_size,
    ...
    for (data_size in data_size_vec) {
      ...
      for (i in 1:try_size) {
        ...
        res_blacklist <- getBlacklistWithFisher(sample_data)
        res_learn <- hc(sample_data, score = "bic", blacklist = res_blacklist)
        ...
        precision_vec <- append(precision_vec,
                               diff_table$tp / (diff_table$tp + diff_table$fp))
      }
...
main <- function() {
  ...
  result <- learnBN(insurance,
                    ...
                    correct_graph_string,
                    flag_skeleton = FALSE)
  plotPrecisionResult(data_size_vec, "insurance", 100 * result[[7]], 100 *
                        result[[8]])
