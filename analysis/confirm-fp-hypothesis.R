...
compareAndReturnTPFPFN <-
  function(res.correct,
           sample.size.vec,
           try.size,
           data,
           skeleton = FALSE) {
    ...
    for (sample.size in sample.size.vec) {
      ...
      for (i in 1:try.size) {
        ...
        diff.table = compare(target = res.correct,
                             current = res.learn,
                             arcs = FALSE)
...
getCsvData <-
  function(data.size.num.vec,
           tp.parcent.vec,
           fp.parcent.vec,
           fn.parcent.vec) {
    data.vec = c(tp.parcent.vec, fp.parcent.vec, fn.parcent.vec)
    print(data.vec)
    data.csv.matrix = matrix(data.vec, nrow = length(data.vec) / 3, ncol =
                               3)
...
outputResult <-
  function(result,
    ...
    plotResult(
      ...
      "data size",
      "tp",
      paste("Data Name : ", data.name, ", Try Number : ", try.size)
    )
    dev.off()
    png(paste("Data Name ", data.name, ", Try Number ", try.size, ".png"))
    plotResult(
    ...
    data.csv.matrix = getCsvData(sample.size.vec,
                                 tp.parcent.vec,
                                 fp.parcent.vec,
                                 fn.parcent.vec)
    print(data.csv.matrix)
...
main <- function(try.size = 50,
  ...
  asia.size.vec = seq(10, 50, by = 5)
  asia.correct.graph.string = "[A][S][T|A][L|S][B|S][D|B:E][E|T:L][X|E]"
  asia.result.undirected = checkData(asia,
                                     asia.size.vec,
                                     try.size,
                                     asia.correct.graph.string,
                                     "asia",
                                     skeleton = TRUE)
  asia.result.directed   = checkData(asia,
                                     asia.size.vec,
                                     try.size,
                                     asia.correct.graph.string,
                                     "asia",
                                     skeleton = FALSE)
  outputResult(asia.result.undirected,
               asia.size.vec,
               "asia.undirected",
               try.size)
  outputResult(asia.result.directed,
               asia.size.vec,
               "asia.directed",
               try.size)
  outputFPResultwithOverride(
    asia.result.directed[[2]],
    "blue",
    asia.result.undirected[[2]],
    "red",
    asia.size.vec,
    "asia fp",
    try.size
  ...
  # insurance
  data(insurance)
  insurance.correct.graph.string = paste(
  ...
  insurance.size.vec = seq(10, 50, by = 5)
  insurance.result.undirected = checkData(
    insurance,
    insurance.size.vec,
    try.size,
    insurance.correct.graph.string,
    "insurance",
    skeleton = TRUE
  )
  insurance.result.directed = checkData(
    insurance,
    insurance.size.vec,
    try.size,
    insurance.correct.graph.string,
  ...
  outputResult(
    insurance.result.undirected,
    insurance.size.vec,
    "insurance.undirected",
    try.size
  )
  outputResult(insurance.result.directed,
               insurance.size.vec,
               "insurance.directed",
               try.size)
  outputFPResultwithOverride(
    insurance.result.directed[[2]],
    "blue" ,
    insurance.result.undirected[[2]],
    "red",
    insurance.size.vec,
    "insurance fp",
    try.size
  ...
  if (FALSE) {
    # check alarm
    data(alarm)
    alarm.correct.graph.string = paste(
    ...
    alarm.size.vec = seq(10, 50, by = 5)
    alarm.result.undirected = checkData(alarm,
                                        alarm.size.vec,
                                        try.size,
                                        alarm.correct.graph.string,
                                        "alarm",
                                        skeleton = TRUE)
    alarm.result.directed = checkData(alarm,
                                      alarm.size.vec,
                                      try.size,
                                      alarm.correct.graph.string,
                                      "alarm",
                                      skeleton = FALSE)
    outputResult(alarm.result.undirected,
                 alarm.size.vec,
                 "alarm.undirected",
                 try.size)
    outputResult(alarm.result.directed,
                 alarm.size.vec,
                 "alarm.directed",
                 try.size)
    outputFPResultwithOverride(
      alarm.result.directed[[2]],
      "blue",
      alarm.result.undirected[[2]],
      "red",
      alarm.size.vec,
      "alarm fp",
      try.size
    ...
    clgaussian.test.result.undirected = checkData(
      clgaussian.test,
      clgaussian.test.size.vec,
      try.size,
      clgaussian.test.correct.graph.string,
      "clgaussian.test",
      skeleton = TRUE
    )
    clgaussian.test.result.directed = checkData(
      clgaussian.test,
      clgaussian.test.size.vec,
      try.size,
      clgaussian.test.correct.graph.string,
    ...
    outputResult(
      clgaussian.test.result.undirected,
      clgaussian.test.size.vec,
      "clgaussian.test.undirected",
      try.size
    )
    outputResult(
      clgaussian.test.result.directed,
      clgaussian.test.size.vec,
      "clgaussian.test.directed",
      try.size
    )
    outputFPResultwithOverride(
      clgaussian.test.result.directed[[2]],
      "blue",
      clgaussian.test.result.undirected[[2]],
      "red",
      clgaussian.test.size.vec,
      "clgaussian.test fp",
      try.size
    ...
    gaussian.test.result.undirected = checkData(
      gaussian.test,
      gaussian.test.size.vec,
      try.size,
      gaussian.test.correct.graph.string,
      "gaussian.test",
      skeleton = TRUE
    )
    gaussian.test.result.directed = checkData(
      gaussian.test,
      gaussian.test.size.vec,
      try.size,
      gaussian.test.correct.graph.string,
    ...
    outputResult(
      gaussian.test.result.undirected,
      gaussian.test.size.vec,
      "gaussian.test.undirected",
      try.size
    )
    outputResult(
      gaussian.test.result.directed,
      gaussian.test.size.vec,
      "gaussian.test.directed",
      try.size
    )
    outputFPResultwithOverride(
      gaussian.test.result.directed[[2]],
      "blue" ,
      gaussian.test.result.undirected[[2]],
      "red",
      gaussian.test.size.vec,
      "gaussian.test fp",
      try.size
    ...
    learning.test.result.undirected = checkData(
      learning.test,
      learning.test.size.vec,
      try.size,
      learning.test.correct.graph.string,
      "learning.test",
      skeleton = TRUE
    )
    learning.test.result.directed = checkData(
      learning.test,
      learning.test.size.vec,
      try.size,
      learning.test.correct.graph.string,
    ...
    outputResult(
      learning.test.result.undirected,
      learning.test.size.vec,
      "learning.test.undirected",
      try.size
    )
    outputResult(
      learning.test.result.directed,
      learning.test.size.vec,
      "learning.test.directed",
      try.size
    )
    outputFPResultwithOverride(
      learning.test.result.directed[[2]],
      "blue" ,
      learning.test.result.undirected[[2]],
      "red",
      learning.test.size.vec,
      "learning.test fp",
      try.size
    ...
    lizards.result.undirected = checkData(
      lizards,
      lizards.size.vec,
      try.size,
      lizards.correct.graph.string,
      "lizards",
      skeleton = TRUE
    )
    lizards.result.directed = checkData(
      lizards,
      lizards.size.vec,
      try.size,
      lizards.correct.graph.string,
    ...
    outputResult(lizards.result.undirected,
                 lizards.size.vec,
                 "lizards.undirected",
                 try.size)
    outputResult(lizards.result.directed,
                 lizards.size.vec,
                 "lizards.directed",
                 try.size)
    outputFPResultwithOverride(
      lizards.result.directed[[2]],
      "blue" ,
      lizards.result.undirected[[2]],
      "red",
      lizards.size.vec,
      "lizards fp",
      try.size
    ...
    #hailfinder
    data(hailfinder)
    hailfinder.correct.graph.string = paste(
    ...
    hailfinder.result.undirected = checkData(
      hailfinder,
      hailfinder.size.vec,
      try.size,
      hailfinder.correct.graph.string,
      "hailfinder",
      skeleton = TRUE
    )
    hailfinder.result.directed = checkData(
      hailfinder,
      hailfinder.size.vec,
      try.size,
      hailfinder.correct.graph.string,
    ...
    outputResult(
      hailfinder.result.undirected,
      hailfinder.size.vec,
      "hailfinder.undirected",
      try.size
    )
    outputResult(
      hailfinder.result.directed,
      hailfinder.size.vec,
      "hailfinder.directed",
      try.size
    )
    outputFPResultwithOverride(
      hailfinder.result.directed[[2]],
      "blue" ,
      hailfinder.result.undirected[[2]],
      "red",
      hailfinder.size.vec,
      "hailfinder fp",
      try.size
